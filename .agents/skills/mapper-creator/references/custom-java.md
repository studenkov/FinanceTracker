...
3. Add toEntity static method from `examples/_fragments/custom-to-entity/java.md`
    - For each direct DTO field: extract value into a local using the DTO accessor (`getX()` for class DTOs, `x()` for
      record DTOs)
    - Create new entity instance
    - Call entity setters with the locals
    - For flat ToOne fields: build a JPA stub of the association entity (`new PetType(); petType.setId(...);`) and pass
      it to the parent setter
    - Return the entity

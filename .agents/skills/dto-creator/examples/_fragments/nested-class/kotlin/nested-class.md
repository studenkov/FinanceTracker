...
- KDoc is **always** present, also using FQN form `[fully.qualified.EntityName]` (same rule as the top-level class — no
  short-name + import shortcut).
- The nested class is `data class`, not `class` — Kotlin always emits data classes for nested DTOs too.
- Auto-suffix on collision still applies: if a `PetDto` already exists somewhere accessible (top-level in the same
  package, or another nested), use `PetDto1`, `PetDto2`, etc.
- When an association becomes a nested class, auto-include all scalar children and leave sub-associations out by
  default.
